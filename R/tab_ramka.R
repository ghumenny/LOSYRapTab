...
#' @importFrom dplyr select bind_rows setdiff mutate filter rowwise ungroup
...
generuj_ramke_s7_parametry <- function(glowna_edycja) {
  ...
  ramka_s7 <- bind_rows(ramka_s7_ograniczone_szkoly, ramka_s7_pozostale_szkoly) %>%
    ...
    select(wskaznik, WOJ_NAZWA, typ_szk2, rok_abs, edycja, kryterium, rok, miesiac, wynik)
...
#' @importFrom dplyr select mutate bind_rows setdiff intersect
#' @importFrom tidyr expand_grid
#' @export
generuj_ramke_k1_parametry <- function(glowna_edycja) {
  ...
  #    "kategoria_k1_2" = c("nauka_bs2st", "nauka_lodd", "nauka_kkzf")
  #  )
